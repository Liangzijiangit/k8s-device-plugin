...
const (
	...
	MigStrategyMixedFullyQualified        = "mixed-fully-qualified"
...
func NewMigStrategy(strategy string) (MigStrategy, error) {
	...
	case MigStrategyMixedFullyQualified:
		return &migStrategyMixedFullyQualified{}, nil
...
type migStrategyMixedFullyQualified struct{}
...
func (s *migStrategySingle) getResourceName(mig *nvml.Device) string {
	...
	r := fmt.Sprintf("mig-%d.%dg.%dgb", c, g, gb)
...
func migStrategyMixedGetPlugins(s MigStrategy, resources MigStrategyResourceSet) []*NvidiaDevicePlugin {
	plugins := []*NvidiaDevicePlugin{
		NewNvidiaDevicePlugin(
			...
			"NVIDIA_VISIBLE_DEVICES_FULL_GPUS",
			pluginapi.DevicePluginPath+"nvidia-gpu.sock"),
	}

	for resource := range resources {
		slug := strings.ToUpper(resource)
		slug = strings.Replace(slug, "-", "_", -1)
		slug = strings.Replace(slug, ".", "_", -1)
		plugin := NewNvidiaDevicePlugin(
			"nvidia.com/"+resource,
			NewMigDeviceManager(s, resource),
			"NVIDIA_VISIBLE_DEVICES_"+slug,
...
// migStrategyMixedFullyQualified
func (s *migStrategyMixedFullyQualified) GetPlugins() []*NvidiaDevicePlugin {
	resources := make(MigStrategyResourceSet)
	for _, mig := range getAllMigDevices() {
		r := s.getResourceName(mig)
		resources[r] = struct{}{}
...
func (s *migStrategyMixedFullyQualified) getResourceName(mig *nvml.Device) string {
	attr, err := mig.GetAttributes()
	check(err)
...
func (s *migStrategyMixedFullyQualified) MatchesResource(mig *nvml.Device, resource string) bool {
